...
for ite in range(iteration):
    ...
    min_dist_loc = np.argmin(dist_cost)
    min_dist_cost = dist_cost[min_dist_loc]
    best_route = routes[min_dist_loc,:]   
    ...
    for i in range(m):
        for j in range(n-1):
            ...
            delta_distance = 1/dist_cost[i]
            pheromone[int(routes_opt[i,j])-1,int(routes_opt[i,j+1])-1] = pheromone[int(routes_opt[i,j])-1,int(routes_opt[i,j+1])-1] + delta_distance
         
# print cost and best route
print("All routes:")
print(routes_opt)
print("Best path: ", best_route)
print('Cost of the best path: ',int(min_dist_cost[0]) + d[int(best_route[-2])-1,0])